...
<div class="card mb-3">
    ...
    <div style="max-height: 350px; overflow:hidden">
        <img class="img-fluid" src="{{ asset('storage/' . $posts[0]->image) }}"
            alt="http://source.unsplash.com/500x400?{{ $posts[0]->category->name }}">
    </div>
    @else
...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            <div class="card">
                <div class="position-absolute px-3 py-2" style="background-color: rgb(0,0,0,0.7)">
                    <a class="text-white text-decoration-none"
                        href="/posts?category={{ $post->category->slug }}">{{ $post->category->name }}</a></div>
                @if ($post->image)
