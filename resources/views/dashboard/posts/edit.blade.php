...
<div class="col-lg-8">
    <form action="/dashboard/posts/{{ $post->slug }}" method="post" class="mb-5" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug" id='slug' required value="{{ Str::slug(old('title', $post->title)) }}">
            @error('slug')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="mb-3">
            ...
            @if ($post->image)
                <img src="{{ asset('storage/' . $post->image) }}" class="img-preview d-block img-fluid col-lg-6 mb-1">
            @else
                <img class="img-preview img-fluid col-lg-6 mb-1">
            @endif
...
<script>
    ...
    //     .then(data => slug.value = slug.data)
    // })
